  ...
  const displayMenu = menuItems
    .map((items) => {
      return `<article class="menu-item">
          <img src=${items.img} alt=${items.img} class="photo" />
          <div class="item-info">
            <header>
              <h4>${items.title}</h4>
...
const displayFilterBtns = function () {
  ...
  const displayCategoryBtns = categories
    .map((category) => {
      return `<button class="filter-btn" type="button" data-id=${category}>${category}</button>`;
    })
    .join("");
  btnContainer.innerHTML = displayCategoryBtns;
  ...
  filterBtns.forEach((btn) => {
    btn.addEventListener("click", function (e) {
      ...
      const menuCategory = menu.filter((item) => {
        if (item.category === category) return item.category;
      });

      if (category === "all") displayMenuItems(menu);
      if (category !== "all") displayMenuItems(menuCategory);
    });
  });
};
